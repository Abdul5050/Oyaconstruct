 ...
 */
function rigid_optionsframework_tabs() {
 ...
 */
function rigid_optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		switch ($value['type']) {
			...
			case "images":
				...
				foreach ($value['options'] as $key => $option) {
					...
					$output .= '<span class="of-radio-img-img' . esc_attr($selected) . '" onclick="document.getElementById(\'' . esc_js($value['id'] . '_' . $key) . '\').checked=true;"><img src="' . esc_url($option) . '" /></span>';
				}
				break;
			...
			case "checkbox":
				...
				$output .= '<label class="explain" for="' . esc_attr($value['id']) . '">' . wp_kses($explain_value, $desc_allowed_html) . '</label>';
				break;
			...
			// Uploader
			case "upload":
				$output .= rigid_optionsframework_medialibrary_uploader($value['id'], $val, null);
				break;

			// rigid Uploader
			case "rigid_upload":
				$is_multiple = array_key_exists('is_multiple', $value) && $value['is_multiple'] ? true : false;
				$output .= rigid_medialibrary_uploader($value['id'], $val, null, '', 0, '', $is_multiple);
				break;

			// Typography
			case 'typography':

				unset($font_size, $font_style, $font_face, $font_color, $font_preview);
				...
				$typography_options = array(
						'sizes' => rigid_recognized_font_sizes(),
						'faces' => rigid_recognized_font_faces(),
						'styles' => rigid_recognized_font_styles(),
						'color' => true,
						'preview' => false
				);
				...
				if ($typography_options['preview']) {
					...
					if ($value['id'] == 'body_font') {
						...
						$headings_font = rigid_get_option('headings_font', array('face' => 'Oswald'));
					...
					$font_preview = '<div id="' . esc_attr($value['id']) . '_preview" class="rigid_font_preview">';
				...
				$typography_fields = apply_filters('rigid_typography_fields', $typography_fields, $typography_stored, $option_name, $value);
			...
			case 'background':
				...
				$output .= rigid_medialibrary_uploader($value['id'], $background['image'], null, '', 0, 'image', false);
				...
				$repeats = rigid_recognized_background_repeat();
				...
				$positions = rigid_recognized_background_position();
				...
				$attachments = rigid_recognized_background_attachment();
			...
			// Editor
			case 'editor':
				$output .= '<div class="explain">' . wp_kses($explain_value, $desc_allowed_html) . '</div>' . "\n";
			...
			case "info":
				...
				if ($value['desc']) {
					$output .= apply_filters('rigid_sanitize_info', $value['desc']) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
			...
			case "start_section":
				...
				if (isset($value['name'])) {
					$output .= '<h4 class="acc-heading"><strong>' . esc_html($value['name']) . '</strong></h4>' . "\n";
				}
			...
			case "heading":
				...
				$jquery_click_hook = "of-option-" . $jquery_click_hook;
				$menu .= '<a id="' . esc_attr($jquery_click_hook) . '-tab" class="nav-tab" title="' . esc_attr($value['name']) . '" href="' . esc_url('#' . $jquery_click_hook) . '">' . esc_html($value['name']) . '</a>';
				$output .= '<div class="' . esc_attr($class) . '" id="' . esc_attr($jquery_click_hook) . '">';
			...
			case 'sidebar':
				...
				$output .= '<input id="new_' . esc_attr($value['id']) . '" class="of-input" name="new_' . esc_attr($option_name . '[' . $value['id'] . ']') . '" type="text" value="" />';
				$output .= '<input type="button" value="' . esc_html__('Add', 'rigid') . '" class="button" id="add_custom_sidebar">';
				$output .= '<input id="' . esc_attr($value['id']) . '" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" type="hidden" value="' . esc_attr($val) . '" />';

				if (count($custom_sidebars)) {
					$output .= '<h4>' . esc_html__('List of created custom sidebars:', 'rigid') . '</h4>';

					$output .= '<div id="rigid_custom_sidebars_list">';

					foreach ($custom_sidebars as $sidebar) {
						$output .= '<div><a class="rigid_del_sdbr" data-sidebar-name="' . $sidebar . '" href="#"></a> ' . $sidebar . '</div>';
					}

					$output .= '</div>';
				} else {
					$output .= '<h4 class="rigid_no_custom_sidebars">' . esc_html__('No custom sidebars.', 'rigid') . '</h4>';
					$output .= '<div id="rigid_custom_sidebars_list"></div>';
				}
		...
		echo $output; // All dynamic data escaped. Rendered in the backend.
	}
	echo '</div>';
}
