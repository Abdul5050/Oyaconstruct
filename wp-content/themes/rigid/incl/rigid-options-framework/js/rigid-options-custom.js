...
jQuery(document).ready(function ($) {
	...
	$('input.expandable_option').click(unhideHidden);
	...
	$('div.import_rigid_demo span.of-radio-img-img').click(function () {
		...
		$action = 'rigid_import_' + $option_el.attr('id').substring(15);

		$(this).before('<span id="rigid_import_spinner" class="rigid_import_spinner"></span>');
		$spinner_el = $("#rigid_import_spinner");
		$spinner_el.click(function (event) {
		...
		$.post(ajaxurl, data, function (response) {
			...
			import_done_indx = response.search("rigid_import_done");
			...
			if ((xhr.status === 500 || xhr.status === 503) && import_run_attempts <= 10) {
				...
				$option_el.find("span.of-radio-img-img").click();
			} else {
	...
	$('div.import_rigid_demo span.of-radio-img-img').hover(function () {
		if (!$('span.rigid_import_spinner').length) {
			$(this).prepend('<span class="rigid_import_btn">Import</span>');
		}
	}, function () {
		$(this).find('span.rigid_import_btn').remove();
	});
});