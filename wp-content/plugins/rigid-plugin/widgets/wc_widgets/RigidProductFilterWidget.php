...
/**
 * Class RigidProductFilterWidget
 *
 * Customized WC_Widget_Layered_Nav WC widget
 * in order to display the swatches
 */
class RigidProductFilterWidget extends WC_Widget {

	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->widget_cssclass    = 'woocommerce widget_layered_nav rigid_product_filter';
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories, using default attribute types.', 'rigid-plugin' );
		$this->widget_id          = 'rigid_product_filter_widget';
		$this->widget_name        = __( 'Rigid Product Filter', 'rigid-plugin' );
		parent::__construct();
	}
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'rigid-plugin' ),
				'label' => __( 'Title', 'rigid-plugin' ),
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => __( 'Attribute', 'rigid-plugin' ),
				'options' => $attribute_array,
			),
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'rigid-plugin' ),
				'options' => array(
					'and' => __( 'AND', 'rigid-plugin' ),
					'or'  => __( 'OR', 'rigid-plugin' ),
				),
			),
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$attribute_object = Rigid_WCVS()->get_tax_attribute( $taxonomy );
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label            = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'rigid-plugin' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
	...
	protected function layered_nav_custom( $att_type, $terms, $taxonomy, $query_type ) {
		// List display
		echo '<div class="rigid-wcs-swatches">';
		...
		foreach ( $terms as $term ) {
			...
			switch ( $att_type ) {
				...
				case 'image':
					...
					$image = $image ? wp_get_attachment_image_src( $image, 'rigid-widgets-thumb' ) : '';
...
add_action('widgets_init', 'rigid_register_rigid_product_filter_widget');
if (!function_exists('rigid_register_rigid_product_filter_widget')) {

	function rigid_register_rigid_product_filter_widget() {
		register_widget('RigidProductFilterWidget');
	}
