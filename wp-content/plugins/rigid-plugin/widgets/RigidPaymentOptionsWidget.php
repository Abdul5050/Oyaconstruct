 ...
 * @author aatanasov
 */
class RigidPaymentOptionsWidget extends WP_Widget{
    public function __construct() {
        $widget_ops = array('description' =>esc_html__('Display a list of available payment methods and seal code', 'rigid-plugin') );
        parent::__construct('rigid_payment_options_widget', 'Rigid Payment Options', $widget_ops);
    }
    ...
    public function form( $instance ) {
        //Defaults
        $defaults = array(
            'title'             => esc_html__( 'Payment Options', 'rigid-plugin' ),
        ...
        $labels = array(
            'american-express'  => esc_html__( 'American Express', 'rigid-plugin' ),
            'cirrus'            => esc_html__( 'Cirrus', 'rigid-plugin' ),
            'maestro'           => esc_html__( 'Maestro', 'rigid-plugin' ),
            'mastercard'        => esc_html__( 'Mastercard', 'rigid-plugin' ),
            'visa'              => esc_html__( 'Visa', 'rigid-plugin' ),
            'cash-on-delivery'  => esc_html__( 'Cash on delivery', 'rigid-plugin' ),
            'direct-debit'      => esc_html__( 'Direct Debit', 'rigid-plugin' ),
            'paypal'            => esc_html__( 'PayPal', 'rigid-plugin' ),
            'cheque'            => esc_html__( 'Cheque', 'rigid-plugin' ),
            'google-checkout'   => esc_html__( 'Google checkout', 'rigid-plugin' ),
            'twocheckout'       => esc_html__( '2Checkout', 'rigid-plugin' ),
            'delta'             => esc_html__( 'Delta', 'rigid-plugin' ),
            'discover'          => esc_html__( 'Discover', 'rigid-plugin' ),
            'moneybookers'      => esc_html__( 'Moneybookers', 'rigid-plugin' ),
            'solo'              => esc_html__( 'SOLO', 'rigid-plugin' ),
            'switch'            => esc_html__( 'Switch', 'rigid-plugin' ),
            'western-union'     => esc_html__( 'Western Union', 'rigid-plugin' ),
            'sagepay'           => esc_html__( 'Sage Pay', 'rigid-plugin' )
        );
        ...
        <?php endforeach;  ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'seal' )); ?>"><?php esc_html_e( 'Seal code:', 'rigid-plugin' ); ?></label>
...
add_action('widgets_init', 'rigid_register_rigid_payment_widget');
if ( ! function_exists( 'rigid_register_rigid_payment_widget' ) )
{
	function rigid_register_rigid_payment_widget()
	{
		register_widget('RigidPaymentOptionsWidget');
	}
}
?>
